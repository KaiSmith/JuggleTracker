...
class Ball:
    ...
    def update(self, frame, center):
        ...
        if len(self.positions) >= 3:
            ...
            dy2.append([(dy[-1][0]+dy[-2][0])/2.0, float(dy[-1][1]-dy[-2][1])/(dy[-1][0]-dy[-2][0])])
                ...
                #cv2.circle(f, (self.positions[-1][0],self.positions[-1][1]), 8, (255, 0, 0), thickness=3, lineType=8, shift=0)
                #print(dy2[-1][1])
    ...
    def status(self):
        ...
        for i in range(max(0, len(self.dy2)-5), len(self.dy2)):
            ...
            if self.dy2[i][1] < 0:
                score -= 1
        if score >= 3 and self.thrown == False:
            self.thrown = True
            self.throws += 1
        if score <= -3 and self.thrown == True:
            self.thrown = False
            self.catches += 1
            hscore = 0
            ...
            if hscore < 0:
                ...
                self.hands.append('l')
                print('l')
        cv2.putText(f, str(self.catches), (self.positions[-1][0],self.positions[-1][1]), cv2.FONT_HERSHEY_SIMPLEX, 3, (0, 255, 0), 2)
        ...
        for i in range(self.n):
            if self.oldsnapshot[i] == False and snapshot[i] == True:
                if i in self.throworder:
                    self.siteswap.append(self.throworder[::-1].index(i)+1)
                self.throworder.append(i)
        self.oldsnapshot = snapshot
        self.draw_count()

    def draw_count(self):
        cv2.putText(f, str(self.catches()), (0,75), cv2.FONT_HERSHEY_SIMPLEX, 3, (0, 255, 0), 2)

balls = [Ball((255, 0, 0)), Ball((0, 0, 255)), Ball((255, 0, 255))]
...
def updateBalls(frame, blobs, balls):
    ...
    if len(scores) > 0:
        for d in range(len(balls)):
            b = min(scores)
            n = scores.index(b)
            if dist(balls[n/len(blobs)].predict(), blobs[n%len(blobs)]) < 150:
...
while(1):
    ...
    thresh = cv2.inRange(hsv,np.array([8, 100, 200],np.uint8),np.array([25, 255, 255],np.uint8))
    kernel = np.ones((5, 5), np.uint8)
    thresh = cv2.erode(thresh, kernel, iterations = 1)
    thresh = cv2.dilate(thresh, kernel, iterations = 2)
    t = np.copy(thresh)
    #thresh = cv2.inRange(hsv,np.array([5, 50, 220],np.uint8),np.array([25, 200, 255],np.uint8))
    #thresh = cv2.inRange(hsv,np.array([0, 50, 240],np.uint8),np.array([25, 150, 255],np.uint8))
...
pplot.scatter([x[0] for x in balls[bn].dy], [x[1] for x in balls[bn].dy], c='g')
pplot.scatter([x[0] for x in balls[bn].dy2], [0 if x[1]<=0 else 1 for x in balls[bn].dy2], c='b')
pplot.show()
